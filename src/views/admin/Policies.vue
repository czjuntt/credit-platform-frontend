<template>
  <div class="page-container">
    ...
    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑政策' : '新增政策'" width="600px">
      <el-form :model="form" :rules="rules" ref="formRef" label-width="100px">
        ...
        <el-form-item label="附件">
          <el-input v-model="form.file_path" placeholder="附件链接（可选）" />
        </el-form-item>
      </el-form>
      <template #footer>
    ...
    <el-dialog v-model="auditVisible" title="审核政策" width="500px">
...
import { getPolicies, createPolicy, updatePolicy, deletePolicy, auditPolicy, getBanks } from '../../api'
...
const form = ref({ id: null, title: '', content: '', bank_id: null, file_path: '' })
...
async function loadData() {
  loading.value = true
  try {
    const [itemsData, banksData] = await Promise.all([getPolicies(), getBanks()])
...
function handleAdd() {
  isEdit.value = false
  form.value = { title: '', content: '', bank_id: banks.value[0]?.id || null, file_path: '' }
...
async function handleSubmit() {
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      try {
        if (isEdit.value) {
          await updatePolicy(form.value.id, form.value)
          ElMessage.success('更新成功')
        } else {
          await createPolicy({ ...form.value, create_user_id: 1 })
...
async function submitAudit() {
  try {
    await auditPolicy(auditForm.value.id, {
